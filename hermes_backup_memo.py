...
VOICE_CONFIG = {
    'FR': {
        ...
        'out_prod': "hermes_fr_master.mp3",
        'out_test': "hermes_fr_dry_run.mp3",
        'params': {
            "stability": 0.45,
    ...
    'EN': {
        ...
        'out_prod': "hermes_en_master.mp3",
        'out_test': "hermes_en_dry_run.mp3",
        'params': {
            "stability": 0.5,
...
# --- 4. MOTEUR DE GÉNÉRATION AUTOMATIQUE ---
def generate_hermes():
    # SÉCURITÉ ULTRA-VISIBLE POUR SAVOIR CE QUI CLOCHE
    ...
    if TEST_MODE:
        output_dir = BASE_PATH / "cognition" / "output" / "test"
        file_name = config['out_test']
        mode_label = f"🔬 MODE : TEST ACTIVE [{LANGUE}]"
    else:
        output_dir = BASE_PATH / "cognition" / "output" / "mastered"
        file_name = config['out_prod']
    ...
    print(f"{mode_label} | 🛡️  Auto-backup actif. Envoi de la requête...")
    response = requests.post(url, json=data, headers=headers)

    if response.status_code == 200:
        archive_name = f"{ts}_{file_name}"
        ...
        export_path_standard = output_dir / file_name
        export_path_archive = output_dir / archive_name

        normalized_audio.export(export_path_standard, format="mp3")
        normalized_audio.export(export_path_archive, format="mp3")
            
        print(f"✅ ÉCRITURE PHYSIQUE RÉUSSIE :")
        print(f" -> {export_path_standard.resolve()}")
        print(f" -> {export_path_archive.resolve()}")
    else:
